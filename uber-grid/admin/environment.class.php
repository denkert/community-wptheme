...
class UberGrid_Environment extends Karevn_Environment_1_0{
	...
	function _init(){
		new Karevn_Support_1_0($this, array(
						...
						'api_url' => apply_filters('ubergrid_api_url', 'http://api.karevn.com/tickets/'),
	...
	function get_diagnostic_data(){
		...
		$data []= array(
				...
				'error' => is_wp_error($http_status) ? $http_status->get_error_message() .
								__('Load more will not work', 'uber-grid') : null
		);
		$data []= array(
		...
		$data []=
				array(
						...
						'warning' => true,
						'error' => __('Cannot create image cache. Please create wp-content/uber-grid-cache directory manually and allow a write access there', 'uber-grid'));
		$data []=
				array(
						'name' => 'wp-content writable',
						'status' => is_writable(WP_CONTENT_DIR),
						'warning' => true,
						'error' => __('wp-content directory is not writable, Images may not show right', 'uber-grid'));
		...
		$data []=
				array(
						...
						'error' => __('Uploads dir renamed.'));
		return $data;
	}

	function get_main_cache_dir_available(){
		return file_exists(ubergrid_get_main_cache_dir()) && is_writable(ubergrid_get_main_cache_dir()) ||
		is_writable(dirname(ubergrid_get_main_cache_dir()));
	}

	function get_backup_cache_dir_available(){
		return file_exists(ubergrid_get_backup_cache_dir()) && is_writable(ubergrid_get_backup_cache_dir()) ||
		is_writable(dirname(ubergrid_get_backup_cache_dir()));
	}
	...
	function get_cache_dir_available(){
		...
		if (!$this->get_main_cache_dir_available()){
			$this->try_to_create(uber_grid_get_main_cache_dir());
			if ($this->get_main_cache_dir_available())
				return true;
		}
		if (!$this->get_backup_cache_dir_available()){
			$this->try_to_create(uber_grid_get_backup_cache_dir());
			if ($this->get_backup_cache_dir_available())
				return true;
		}
		return false;
	...
	function get_uploads_dir_not_renamed(){
		if (uber_grid_is_photon_enabled())
			return true;
		if ($this->get_main_cache_dir_available() == true)
			return true;
		if (get_option('upload_path') == "wp-content/uploads" || !get_option('upload_path')
				|| trailingslashit(get_option('upload_path')) ==  trailingslashit(trailingslashit(WP_CONTENT_DIR) . "uploads"))
			return true;
		return sprintf(__("Your uploads dir was renamed to %s. Please create <code>wp-content/uber-grid-timthumb-config.php</code> and add the next <code>%s</code> line at the top of that file.", $this->namespace),
	...
	function get_errors(){
		...
		if (!$this->get_gd_supported()){
			$errors []= __('UberGrid requires GD library in order to work correctly. Please contact your hosting administrator.', 'uber_grid');
		}
		return $errors;
	}
